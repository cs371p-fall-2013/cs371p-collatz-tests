// --------------------------------
// projects/collatz/TestCollatz.c++
// Copyright (C) 2013
// Glenn P. Downing
// --------------------------------

/*
To test the program:
% ls -al /usr/include/gtest/
...
gtest.h
...

% locate libgtest.a
/usr/lib/libgtest.a

% locate libpthread.a
/usr/lib/x86_64-linux-gnu/libpthread.a
/usr/lib32/libpthread.a

% locate libgtest_main.a
/usr/lib/libgtest_main.a

% g++ -pedantic -std=c++0x -Wall Collatz.c++ TestCollatz.c++ -o TestCollatz -lgtest -lpthread -lgtest_main

% valgrind TestCollatz > TestCollatz.out
*/

// --------
// includes
// --------

#include <iostream> // cout, endl
#include <sstream> // istringtstream, ostringstream
#include <string> // ==

#include "gtest/gtest.h"

#include "Collatz.h"

// -----------
// TestCollatz
// -----------

// ----
// read
// ----

TEST(Collatz, read_1) {
    std::istringstream r("1 10\n");
    int i;
    int j;
    const bool b = collatz_read(r, i, j);
    ASSERT_TRUE(b == true);
    ASSERT_TRUE(i == 1);
    ASSERT_TRUE(j == 10);}

TEST(Collatz, read_2) {
    std::istringstream r("100 200\n");
    int i;
    int j;
    const bool b = collatz_read(r, i, j);
    ASSERT_TRUE(b == true);
    ASSERT_TRUE(i == 100);
    ASSERT_TRUE(j == 200);}

TEST(Collatz, read_3) {
    std::istringstream r("201 210\n");
    int i;
    int j;
    const bool b = collatz_read(r, i, j);
    ASSERT_TRUE(b == true);
    ASSERT_TRUE(i == 201);
    ASSERT_TRUE(j == 210);}

TEST(Collatz, read_4) {
    std::istringstream r("900 1000\n");
    int i;
    int j;
    const bool b = collatz_read(r, i, j);
    ASSERT_TRUE(b == true);
    ASSERT_TRUE(i == 900);
    ASSERT_TRUE(j == 1000);}

TEST(Collatz, read_5) {
    std::istringstream r("50 1\n");
    int i;
    int j;
    const bool b = collatz_read(r, i, j);
    ASSERT_TRUE(b == true);
    ASSERT_TRUE(i == 50);
    ASSERT_TRUE(j == 1);}

TEST(Collatz, read_6) {
    std::istringstream r("4\n");
    int i;
    int j;
    const bool b = collatz_read(r, i, j);
    ASSERT_TRUE(b == false);}

TEST(Collatz, read_7) {
    std::istringstream r("\n");
    int i;
    int j;
    const bool b = collatz_read(r, i, j);
    ASSERT_TRUE(b == false);}

// ----
// eval
// ----

TEST(Collatz, eval_1) {
    const int v = collatz_eval(1, 10);
    ASSERT_TRUE(v == 20);}

TEST(Collatz, eval_2) {
    const int v = collatz_eval(100, 200);
    ASSERT_TRUE(v == 125);}

TEST(Collatz, eval_3) {
    const int v = collatz_eval(201, 210);
    ASSERT_TRUE(v == 89);}

TEST(Collatz, eval_4) {
    const int v = collatz_eval(900, 1000);
    ASSERT_TRUE(v == 174);}

TEST(Collatz, eval_5) {
    const int v = collatz_eval(1, 1);
    ASSERT_TRUE(v == 1);}

TEST(Collatz, eval_6) {
    const int v = collatz_eval(5, 5);
    ASSERT_TRUE(v == 6);}

TEST(Collatz, eval_7) {
    const int v = collatz_eval(10, 1);
    ASSERT_TRUE(v == 20);}

TEST(Collatz, eval_8) {
    const int v = collatz_eval(1, 999999);
    ASSERT_TRUE(v == 525);}

TEST(Collatz, eval_9) {
    const int v = collatz_eval(999999, 1);
    ASSERT_TRUE(v == 525);}

// -----------
// single_eval
// -----------

TEST(Collatz, single_1) {
    const int v = collatz_single_eval(1);
    ASSERT_TRUE(v == 1);}

TEST(Collatz, single_2) {
    const int v = collatz_single_eval(5);
    ASSERT_TRUE(v == 6);}

TEST(Collatz, single_3) {
    const int v = collatz_single_eval(1000);
    ASSERT_TRUE(v == 112);}

// -----
// print
// -----

TEST(Collatz, print_1) {
    std::ostringstream w;
    collatz_print(w, 1, 10, 20);
    ASSERT_TRUE(w.str() == "1 10 20\n");}

TEST(Collatz, print_2) {
    std::ostringstream w;
    collatz_print(w, 100, 200, 125);
    ASSERT_TRUE(w.str() == "100 200 125\n");}

TEST(Collatz, print_3) {
    std::ostringstream w;
    collatz_print(w, 201, 210, 89);
    ASSERT_TRUE(w.str() == "201 210 89\n");}

TEST(Collatz, print_4) {
    std::ostringstream w;
    collatz_print(w, 900, 1000, 174);
    ASSERT_TRUE(w.str() == "900 1000 174\n");}

TEST(Collatz, print_5) {
    std::ostringstream w;
    collatz_print(w, 1, 1, 1);
    ASSERT_TRUE(w.str() == "1 1 1\n");}

TEST(Collatz, print_6) {
    std::ostringstream w;
    collatz_print(w, 0, 10, 20);
    ASSERT_TRUE(w.str() == "0 10 20\n");}

TEST(Collatz, print_7) {
    std::ostringstream w;
    collatz_print(w, 1, 1000000, 1);
    ASSERT_TRUE(w.str() == "1 1000000 1\n");}

TEST(Collatz, print_8) {
    std::ostringstream w;
    collatz_print(w, 10, 1, 20);
    ASSERT_TRUE(w.str() == "10 1 20\n");}

// -----
// solve
// -----

TEST(Collatz, solve) {
    std::istringstream r("1 10\n100 200\n201 210\n900 1000\n");
    std::ostringstream w;
    collatz_solve(r, w);
    ASSERT_TRUE(w.str() == "1 10 20\n100 200 125\n201 210 89\n900 1000 174\n");}

TEST(Collatz, solve_2) {
    std::istringstream r("1 1\n5 5\n1 10\n10 1");
    std::ostringstream w;
    collatz_solve(r, w);
    ASSERT_TRUE(w.str() == "1 1 1\n5 5 6\n1 10 20\n10 1 20\n");}
